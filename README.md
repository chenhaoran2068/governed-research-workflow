...
Status: v1.9.1 new-Study navigation stop-boundary maintenance source, derived
from immutable public v1.9.0. It preserves the frozen v1.0.0 contract,
...
source library, RAG capability, automatic tagging, real mapping decision,
promotion, rule integration, coordinator, agent runtime, delegation,
multi-agent orchestration, generic writer, recovery executor, dependency, or
release decision. `GRW-CAP-180-01` adds generic lifecycle routing, blank
design/classification, governance-readiness, and analysis-state/freeze records,
...
a human-confirmable route recommendation. v1.9.1 clarifies that this route is
the first substantive response, before methods, lifecycle guidance, external
sources, citations, links, data-access suggestions, or project creation. It
does not create a Study, decide a scientific design, infer governance, access
material, execute research, or replace human confirmation. This versioned source tree does not itself prove the release
...
The v0.8.0 release-preparation records are historical pre-C4 snapshots:
...
They describe the evidence, release-scope admission, and later C4 decision
available at that time. They do not override the selected-version
exact-tag-and-matching-Release rule or create a current installation/runtime
claim.
...
### Historical v1.1 Source-Scope Note

Status: v1.1.0 versioned source scope. This historical source-scope note does not itself prove the
release or installation identity. A current candidate or later release must be
...
For a selected released version that admits `GRW-CAP-120-01`, it may provide
generic controlled-experience-vocabulary and reference-index structural review
only when the caller names every metadata JSON file that may be read. It stops
before source content, pointers, discovery, real mapping, vocabulary
...
- `v0.8.0`: historical pre-C4 source record for portable profile boundaries,
  bounded non-runnable role contracts, and controlled bootstrap-helper
  admission; no delegated authority, external-evidence retrieval role, generic
  writer, or multi-agent runtime. Scope admission is not a public-availability
  claim.
...
- `v0.12.0`: no-new-interface synthetic integration assurance across existing
  bounded modules; no actual runtime installation, data/manuscript action, or
  capability admission.
- `v0.13.x+`: pre-v1 scope closure; no automatic feature expansion or public
