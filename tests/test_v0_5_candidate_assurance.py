"""Cross-record assurance for the unreleased v0.5 provenance-register candidate."""
...
class V05CandidateAssuranceTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.ledger = json.loads(LEDGER_PATH.read_text(encoding="utf-8"))
        cls.records = {record["capability_id"]: record for record in cls.ledger["capabilities"]}

    def test_candidate_identity_and_public_baseline_are_separate(self) -> None:
        manifest = MANIFEST_PATH.read_text(encoding="utf-8")
        readme = README_PATH.read_text(encoding="utf-8")
        release_status = CURRENT_RELEASE_PATH.read_text(encoding="utf-8")
        self.assertIn("system_version: 0.5.0-candidate", manifest)
        self.assertIn("unreleased local `v0.5.0` candidate", readme)
        self.assertIn("historical public baseline", readme)
        self.assertIn("local `v0.5.0` candidate", release_status)
        self.assertNotIn("v0.5.0 is released", "\n".join((manifest, readme, release_status)).lower())

    def test_candidate_capability_is_verified_but_forbidden_until_admission(self) -> None:
        ...
        self.assertIn("exact-release admission", record["limitations_and_next_action"])
    ...
    def test_roadmap_and_security_materials_describe_the_same_next_version(self) -> None:
        ...
        self.assertIn("## v0.5.0 (unreleased metadata-only provenance register-set candidate)", roadmap)
        self.assertIn("`v0.6.0`: reviewable workflow and evidence controls", roadmap)
        self.assertIn("`v0.4.x`", security)
        self.assertIn("v0.5 candidate", security)
