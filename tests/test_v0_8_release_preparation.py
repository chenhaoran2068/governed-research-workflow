...
class V08ReleasePreparationTests(unittest.TestCase):
    def test_required_release_materials_exist_and_remain_pre_c4(self) -> None:
        required = (
        ...
        self.assertIn("pre-C4 release-gate record", gate)
        self.assertIn("release-scope admission", admission)
        self.assertIn("separate from C4", admission)
        self.assertIn("does not itself establish publication", notes)

    def test_reviewed_pre_c4_record_validates_and_cannot_stand_in_for_c4(self) -> None:
        ...
        self.assertEqual(record["candidate_identity"]["branch_state"], "main_candidate")
        self.assertEqual(record["candidate_identity"]["exact_commit"], "d896a3f455b37e8c8d757db3687b8b6f13e84d4e")
        self.assertEqual(
            record["capability_set"]["admitted_capability_ids"],
            ["GRW-CAP-080-01", "GRW-CAP-080-02", "GRW-CAP-080-03"],
        )
        self.assertIn("pull-request #10", record["candidate_review_acceptance_reference"])
        self.assertIsNone(record["c4_release_authorization_reference"])
        self.assertIsNone(record["post_release_verification_reference"])
        self.assertEqual(record["release_integrity_posture"]["technical_immutable_release_status"], "enabled")
